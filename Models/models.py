...
class patch_match(nn.Module):
    ...
    def forward(self, x, ref, fold_params={'kernel_size': (7, 7), 'padding': 3, 'stride': 1, 'dilation': 1},
                divisor=None, n=1, hard=False, skip = 1):
        x_feat = (x + 1.) / 2.
        ref = (ref + 1.) / 2.
        ...
        for i in range(n):
            ...
            lrsr_lv1_unfold = F.normalize(lrsr_lv1_unfold, dim=2)  # (N, HlxWl, CxP1xP2)
            R_lv1 = lrsr_lv1_unfold @ refsr_lv1_unfold  # [N, Hl*Wl, Hr*Wr]
            R_lv1 = F.softmax(R_lv1 / 0.01, dim=-1)  # [N, Hl*Wl, Hr*Wr] Get the attention map
            # if i < n - 1:
            #     fusion = torch.bmm(refsr_lv1_unfold,
            #                        R_lv1.permute(0, 2, 1))  # [N,CxP1xP2, HrxWr], [N, Hr*Wr, Hl*Wl] [N, CxP1xP2, Hl*Wl]
            # else:
