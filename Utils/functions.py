...
def R1Penalty(real_img, f, patch = False):
    # gradient penalty
    reals = torch.autograd.Variable(real_img, requires_grad=True).to(real_img.device)
    real_logit = f(reals)
    ...
    if not patch:
        ...
        real_grads = torch.autograd.grad(real_logit, reals, grad_outputs = torch.ones(real_logit.size()).to(reals.device), create_graph=True)[0].view(reals.size(0), -1)
        real_grads = undo_loss_scaling(real_grads)
        r1_penalty = torch.sum(torch.mul(real_grads, real_grads))
        return 10 * r1_penalty
    else:
        real_logit = apply_loss_scaling(real_logit)
        real_grads = \
        torch.autograd.grad(real_logit, reals, grad_outputs = torch.ones(real_logit.size()).to(reals.device),
                            create_graph = True)[0]
        real_grads = undo_loss_scaling(real_grads)
        r1_penalty = torch.mean(torch.mul(real_grads, real_grads))
        return  r1_penalty
...
def adjust_scales2image(real_, opt):
    if opt.pyramid_height is None:
        ...
        opt.scale1 = min(opt.max_size / max([real_.shape[2], real_.shape[3]]),1)  # min(256/max([real_.shape[0],real_.shape[1]]),1)
        real = resize(real_, scale_factor = opt.scale1)
